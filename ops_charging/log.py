...
def getLoggers(name='charging'):
    if name not in _loggers:
        _loggers[name] = OpsLogAdapter(logging.getLogger(name))
    return _loggers[name]

def getLogger(logger_name='charging'):
    logger = logging.getLogger(logger_name)
    logger.setLevel(getattr(logging, "DEBUG"))
    formatter = logging.Formatter(fmt="%(asctime)s %(levelname)s [%(name)s %(lineno)d] %(message)s", \
            datefmt="%Y-%m-%d %H:%M:%S")
    if not os.path.exists('/var/log/charging'):
        os.makedirs('/var/log/charging')
    console = logging.StreamHandler()
    console.setFormatter(formatter)
    filename = "charging"
    filehandler = logging.FileHandler('/var/log/charging/%s.log' % filename)
