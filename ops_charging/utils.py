...
def cleanup_file_locks():
    ...
    lockfile_re = r'ops_charging-.*\.lock'
...
def get_token(username=options.as_dict().get('keystone_username', ''),
              ...
              tenant=options.as_dict().get('keystone_tenant', '')):
    ...
    if tenant:
        ...
        r = get_http(method='post', url='%s/tokens' % options.keystone_admin_endpoint,
...
class ConvertTime2Int(object):
    """convert time to int type. eg: 2017-10-22 22:22:22  convert: 1508601600
    :return int type"""
...
def get_nowtime(need_int=False):
    # when need_int set `True` return timestamp else return str. e.g: 2017-11-33 17:33:33
    if need_int:
